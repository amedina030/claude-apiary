...
def observation(dimension: str, text: str) -> dict:
    return {"dimension": dimension, "observation": text,
            "evidence": "quote", "volatility": "stable"}


def session(sid: str, captured: str, *observations: dict) -> dict:
    return {"session_id": sid, "captured_at": captured,
...
class ReduceLabelTests(unittest.TestCase):
    def test_argmax_wins(self):
        self.assertEqual(
            evaluate.reduce_label("terse and short messages", "communication_style", VOCAB),
            "terse")
    ...
    def test_case_insensitive(self):
        self.assertEqual(
            evaluate.reduce_label("TERSE", "communication_style", VOCAB), "terse")
...
class SessionLabelsTests(unittest.TestCase):
    ...
    def test_unlabelled_dimensions_are_dropped(self):
        s = session("aaaa1111", "2026-01-01T00:00:00Z",
                    observation("autonomy", "no cue in this text"))
        self.assertEqual(evaluate.session_labels(s, VOCAB), {})

    def test_evidence_is_never_read(self):
        s = session("aaaa1111", "2026-01-01T00:00:00Z",
                    {"dimension": "communication_style", "observation": "no cue",
                     "evidence": "terse terse terse", "volatility": "stable"})
        self.assertEqual(evaluate.session_labels(s, VOCAB), {},
                         "evidence is a raw session quote, not the claim")
...
class StubSynthesizeTests(unittest.TestCase):
    def test_emits_a_section_per_populated_dimension(self):
        training = [session("a", "2026-01-02T00:00:00Z",
                            observation("communication_style", "terse"))]
    ...
    def test_is_deterministic(self):
        training = [session("a", "2026-01-02T00:00:00Z",
                    ...
                    session("b", "2026-01-01T00:00:00Z",
                            observation("autonomy", "gate"))]
        self.assertEqual(evaluate.stub_synthesize(training, DIMENSIONS),
                         evaluate.stub_synthesize(training, DIMENSIONS))

    def test_volatile_dimensions_only_see_the_recent_window(self):
        recent = [session(f"s{i}", f"2026-01-{20 - i:02d}T00:00:00Z",
                          observation("mood_tone", "calm"))
                  for i in range(evaluate.STUB_VOLATILE_WINDOW)]
        old = [session("old", "2020-01-01T00:00:00Z",
                       observation("mood_tone", "frustrated"))]
        out = evaluate.stub_synthesize(recent + old, DIMENSIONS)
        self.assertIn("calm", out)
        self.assertNotIn("frustrated", out,
                         "a volatile observation outside the window must not leak in")

    def test_stable_dimensions_see_everything(self):
        training = [session(f"s{i}", f"2026-01-{20 - i:02d}T00:00:00Z",
                            observation("autonomy", "broad"))
...
class EvaluateFoldsTests(unittest.TestCase):
    def _corpus(self, n=6):
        return [session(f"{i:08x}", f"2026-01-{i + 1:02d}T00:00:00Z",
                        observation("communication_style", "terse"),
                        observation("autonomy", "broad"))
                for i in range(n)]
    ...
    def test_a_wrong_profile_scores_zero(self):
        corpus = self._corpus(4)
        result = evaluate.evaluate_folds(
            corpus, VOCAB, DIMENSIONS,
            synthesizer=lambda t: "## communication_style\nverbose\n")
    ...
    def test_random_baseline_reflects_label_count(self):
        result = evaluate.evaluate_folds(
            self._corpus(3), VOCAB, DIMENSIONS,
            synthesizer=lambda t: evaluate.stub_synthesize(t, DIMENSIONS))
        self.assertAlmostEqual(result["random"], 0.5,
                               msg="both scored dimensions are binary")

    def test_per_label_precision(self):
        corpus = self._corpus(4)
        result = evaluate.evaluate_folds(
            corpus, VOCAB, DIMENSIONS,
            synthesizer=lambda t: "## autonomy\ngate\n")
    ...
    def test_max_folds_bounds_the_run(self):
        ...
        evaluate.evaluate_folds(
            self._corpus(6), VOCAB, DIMENSIONS, max_folds=2,
...
class EstimateCostTests(unittest.TestCase):
    def test_scales_with_folds(self):
        corpus = [session(f"{i:08x}", "2026-01-01T00:00:00Z",
                          observation("autonomy", "broad")) for i in range(4)]
...
class CorrectionHeuristicTests(unittest.TestCase):
    def test_detects_common_corrections(self):
        for text in ["no, that's not it", "revert that", "why did you do that",
                     "actually, use B", "undo"]:
...
class SummariseArmsTests(unittest.TestCase):
    def _row(self, sid, turn, task, tokens, message="", timestamp="2026-08-01T00:00:00Z"):
        return {"session_id": sid, "turn_number": turn, "task_turn": task,
                "net_tokens_delta": tokens, "user_message": message,
                "timestamp": timestamp}

    def test_groups_by_arm(self):
        rows = [self._row("on-session", 1, 1, 100),
    ...
    def test_tasks_are_distinct_task_turns(self):
        rows = [self._row("s", 1, 1, 10), self._row("s", 2, 1, 10),
                self._row("s", 3, 2, 10)]
    ...
    def test_corrections_are_counted_once_per_user_turn(self):
        rows = [self._row("s", 1, 1, 10, "no, not like that"),
    ...
    def test_since_filters_rows(self):
        rows = [self._row("s", 1, 1, 10, timestamp="2026-07-01T00:00:00Z"),
                self._row("s", 2, 2, 10, timestamp="2026-08-15T00:00:00Z")]
        summary = evaluate.summarise_arms(rows, since="2026-08-01",
                                          arm_lookup=lambda sid: "on")
        self.assertEqual(summary["arms"]["on"]["rows"], 1)

    def test_empty_arm_reports_none_not_zero(self):
        summary = evaluate.summarise_arms([self._row("s", 1, 1, 10)],
                                          arm_lookup=lambda sid: "on")
        self.assertIsNone(summary["arms"]["off"]["tool_calls_per_task"],
                          "an empty arm must not look like a measured zero")

    def test_rows_without_a_session_id_are_skipped(self):
        summary = evaluate.summarise_arms([{"timestamp": "2026-08-01", "task_turn": 1}],
                                          arm_lookup=lambda sid: "on")
    ...
    def write_sessions(self, n=4):
        for i in range(n):
            payload = session(f"{i:08x}", f"2026-01-{i + 1:02d}T00:00:00Z",
                              observation("communication_style",
                                          "terse single-line directives"),
                              observation("autonomy", "grants broad latitude"))
            (self.observations / f"{i:08x}.json").write_text(
                json.dumps(payload), encoding="utf-8")


class CacheTests(StateDirSandbox):
    def test_round_trip(self):
        evaluate.cache_result({"mode": "stub", "headline": 0.8, "folds": 4,
                               "lift_over_majority": 0.1})
...
class CliTests(StateDirSandbox):
    ...
    def test_offline_human_output_names_the_baselines(self):
        ...
        self.assertEqual(code, 0)
        for expected in ("HEADLINE", "majority baseline", "random baseline",
                         "LIFT over majority", "coverage"):
    ...
    def test_ab_reports_both_arms(self):
        ...
        log.write_text("\n".join(json.dumps(row) for row in [
            {"session_id": "aaaa0001", "turn_number": 1, "task_turn": 1,
             "net_tokens_delta": 10, "timestamp": "2026-08-01T00:00:00Z"},
            {"session_id": "bbbb0002", "turn_number": 1, "task_turn": 1,
             "net_tokens_delta": 20, "timestamp": "2026-08-02T00:00:00Z"},
        ]) + "\n", encoding="utf-8")
