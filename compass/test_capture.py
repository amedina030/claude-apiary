...
class TestStore(CaptureTestCase):
    ...
    def test_full_uuid_session_id_maps_to_the_eight_char_filename(self):
        ...
        result = self.run_cli("store", "--content-file",
                              self.write(make_payload(session_id=full)), "--session-id", full)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(self.stored.is_file())
    ...
    def test_unknown_dimension_is_rejected(self):
        ...
        result = self.run_cli("store", "--content-file", self.write(payload),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 1)
    ...
    def test_empty_observations_are_skipped_by_default(self):
        result = self.run_cli("store", "--content-file",
                              self.write(make_payload(observations=[])), "--session-id", SID)
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_allow_empty_writes_the_file(self):
        result = self.run_cli("store", "--content-file",
                              ...
                              "--session-id", SID, "--allow-empty")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(self.stored.is_file())

    def test_markdown_fences_are_tolerated(self):
        wrapped = "```json\n" + json.dumps(make_payload()) + "\n```"
        result = self.run_cli("store", "--content-file", self.write(wrapped),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(self.stored.is_file())

    def test_invalid_json_is_a_clean_one_line_failure(self):
        result = self.run_cli("store", "--content-file", self.write("{not json"),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 1)
    ...
    def test_missing_file_is_a_clean_failure(self):
        result = self.run_cli("store", "--content-file", str(self.root / "gone.json"),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 1)
    ...
    def test_json_array_payload_is_rejected(self):
        result = self.run_cli("store", "--content-file", self.write("[1, 2]"),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 1)
    ...
    def test_rewrite_replaces_the_previous_capture(self):
        ...
        result = self.run_cli("store", "--content-file", self.write(second, "obs2.json"),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 0, result.stderr)
...
class TestValidate(CaptureTestCase):
    def test_valid_payload_passes(self):
        result = self.run_cli("validate", "--content-file", self.write(make_payload()),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("ok — 1 observation(s)", result.stdout)

    def test_validate_never_writes(self):
        self.run_cli("validate", "--content-file", self.write(make_payload()),
                     "--session-id", SID)
        self.assertFalse(self.stored.exists())

    def test_invalid_payload_lists_every_error(self):
        payload = make_payload()
        payload["observations"][0] = {"dimension": "vibes", "observation": "",
                                      "evidence": "", "volatility": "x"}
        result = self.run_cli("validate", "--content-file", self.write(payload),
                              "--session-id", SID)
        self.assertEqual(result.returncode, 1)
...
class TestDimensionsAndTemplate(CaptureTestCase):
    ...
    def test_template_round_trips_through_validate(self):
        ...
        self.assertEqual(self.run_cli("validate", "--content-file", path,
                                      "--session-id", SID).returncode, 0)
