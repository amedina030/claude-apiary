...
def _run_all(targets):
    ...
    threads = [threading.Thread(target=guarded, args=(fn, *args)) for fn, *args in targets]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
...
class LostUpdateTests(unittest.TestCase):
        ...
        def updater(worker):
            for i in range(self.ADDS_PER_THREAD):
                self.store.update_note(
                    "todo", anchor["year"], anchor["seq"], brief_summary=f"w{worker} pass {i}"
                )

        _run_all(
            [(adder, w) for w in range(self.THREADS)] + [(updater, w) for w in range(self.THREADS)]
        )

        expected = set(added) | {anchor["seq"]}
        missing = expected - self._active_seqs()
        self.assertEqual(missing, set(), f"{len(missing)} index row(s) lost to a concurrent update")
    ...
    def test_archiving_does_not_drop_concurrent_adds(self):
        ...
        victims = [
            self.store.add_note("todo", f"archive me {i}", "s0") for i in range(self.THREADS * 4)
        ]
        added = []
        lock = threading.Lock()
        ...
        _run_all(
            [(adder, w) for w in range(self.THREADS)] + [(archiver, w) for w in range(self.THREADS)]
        )

        active = self._active_seqs()
        missing = set(added) - active
        self.assertEqual(
            missing, set(), f"{len(missing)} index row(s) lost to a concurrent archive"
        )
    ...
    def test_learning_writes_survive_concurrent_archive(self):
        victims = [
            self.store.add_learning(f"archive me {i}", "s0") for i in range(self.THREADS * 4)
        ]
        added = []
        lock = threading.Lock()
        ...
        self.assertEqual(
            set(added) - active, set(), "learning index row lost to a concurrent archive"
        )
...
class TwoProcessAppendTests(unittest.TestCase):
    ...
    def test_two_processes_appending_lose_nothing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn(
                "",
                (year_dir / INDEX_FILENAME).read_text(encoding="utf-8").splitlines(),
                "no blank lines in the index",
            )
