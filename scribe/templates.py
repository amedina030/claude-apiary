...
def overview(state_dir: Path) -> list:
    ...
    return [f'{t:10s} {describe(state_dir, t):60s} {template_path(state_dir, t)}'
            for t in VALID_TYPES if template_text(state_dir, t) is not None]
...
def render(state_dir: Path, note_type: str) -> "str | None":
    ...
    if text is None:
        return None
    return (f'# template for {note_type} ({describe(state_dir, note_type)})\n'
...
def gate(state_dir: Path, note_type: str, content: str, *,
    ...
    if not missing:
        return (None, False)
    if force:
        return (f'[template gate bypassed via --force: {note_type} note missing '
