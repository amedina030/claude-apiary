...
from scribe import formatting as fmt
...
class TestAgeRenderer(unittest.TestCase):
    def test_buckets(self):
        self.assertEqual(fmt.format_age(_ago(seconds=10)), "just now")
...
class TestColorGating(unittest.TestCase):
    def test_plain_by_default(self):
        with mock.patch.dict(os.environ, {}, clear=False):
            ...
            self.assertFalse(fmt.color_enabled())
            self.assertEqual(fmt.colorize("x", "36"), "x")

    def test_force_color_on(self):
        with mock.patch.dict(os.environ, {"FORCE_COLOR": "1"}, clear=False):
            os.environ.pop("NO_COLOR", None)
            self.assertTrue(fmt.color_enabled())
            self.assertIn("\x1b[", fmt.colorize("x", "36"))
