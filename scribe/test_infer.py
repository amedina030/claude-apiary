...
class InferenceSwitchTests(unittest.TestCase):
    ...
    def test_flag_turns_it_on(self):
        self.assertTrue(infer.inference_enabled(
            Namespace(infer=True, no_infer=False), environ={}))

    def test_env_turns_it_on(self):
        for value in ('1', 'true', 'YES', 'on'):
            with self.subTest(value=value):
                self.assertTrue(infer.inference_enabled(
                    Namespace(), environ={infer.INFER_ENV_VAR: value}))

    def test_env_junk_leaves_it_off(self):
        self.assertFalse(infer.inference_enabled(
            Namespace(), environ={infer.INFER_ENV_VAR: 'maybe'}))
...
class ParseResponseTests(unittest.TestCase):
    ...
    def test_plain_json(self):
        self.assertEqual(infer.parse_response('{"tags": ["a"], "areas": []}'),
                         {'tags': ['a'], 'areas': []})
    ...
    def test_normalize_drops_blanks_and_stringifies(self):
        self.assertEqual(infer.normalize({'tags': ['  a  ', '', 7], 'areas': None}),
                         {'tags': ['a', '7'], 'areas': []})
...
class LearnDoesNotCallAModelTests(unittest.TestCase):
    ...
    def _learn_args(self, **overrides):
        base = dict(store=self.store, content='learned a thing', content_file=None,
                    session_id='s1', brief_summary='', role='', mission='',
                    tags='', area=[], supersedes='', infer=False, no_infer=False)
    ...
    def test_infer_flag_makes_the_call(self):
        with mock.patch.object(infer, 'infer_tags_areas',
                               return_value={'tags': ['scribe'], 'areas': ['scribe/**']}) as called:
    ...
    def test_env_opt_in_makes_the_call(self):
        with mock.patch.dict('os.environ', {infer.INFER_ENV_VAR: '1'}), \
             mock.patch.object(infer, 'infer_tags_areas',
                               return_value={'tags': ['env'], 'areas': []}) as called:
            notes_mod.cmd_learn(self._learn_args())
        called.assert_called_once()
    ...
    def test_supplied_tags_are_never_overridden(self):
        with mock.patch.object(infer, 'infer_tags_areas',
                               return_value={'tags': ['guessed'], 'areas': []}) as called:
...
class RetrotagTests(unittest.TestCase):
    ...
    def _fake_infer(self, tags=('inferred',), areas=('scribe/**',)):
        return mock.patch.object(
            infer, 'infer_tags_areas',
            return_value={'tags': list(tags), 'areas': list(areas)})
    ...
    def test_tags_an_untagged_learning_in_index_and_body(self):
        ...
        body = (self.tmp_dir / 'learnings' / str(entry['year'])
                / f"{entry['seq']}.md").read_text(encoding='utf-8')
