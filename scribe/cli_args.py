...
def _add_role_mission(parser, *, filters: bool = False) -> None:
    ...
    if filters:
        ...
        parser.add_argument(
            "--mission", default="", help="Session mission (e.g. general, project-x)"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_template = sub.add_parser(
        "template", help="Inspect per-type formatting templates that gate `add`"
    )
    p_template_sub = p_template.add_subparsers(dest="template_action")
    for action, blurb in (
        ("show", "Print the template content for a type"),
    ...
    p_template_sub.add_parser("list", help="List types whose template file exists and is non-empty")
    ...
    p_list.add_argument(
        "--all", action="store_true", help="Include done, dropped, and deferred notes"
    )
    ...
    _add_id(
        sub.add_parser("get", aliases=["show"]), "Note or learning ID (e.g. T-2026-1, L-2026-3)"
    )
    ...
    sub.add_parser(
        "tidy", help="Run the auto-archive retention sweep now (add and startup run it too)"
    )
    sub.add_parser(
        "mark-reviewed", help="Stamp the learnings review timestamp the startup banner reads"
    )
    ...
    p_learnings.add_argument(
        "--full", action="store_true", help="Print full content (not truncated)"
    )
    ...
    p_learnings.add_argument(
        "--index", action="store_true", help="Compact tag-grouped output for startup injection"
    )

    _add_id(sub.add_parser("unlearn"), "Learning ID (e.g. L-2026-3)")
    _add_id(
        sub.add_parser(
            "archive-learning", help="Archive a learning by ID (move to learnings/<year>/archive/)"
        ),
        "Learning ID (e.g. L-2026-5)",
    )

    p_supersede = sub.add_parser(
        "supersede", help="Archive an existing learning and write a replacement"
    )
    ...
    p_supersede.add_argument(
        "--content-file", dest="content_file", default=None, help="Read the body from a file"
    )
    ...
    p_repair.add_argument(
        "--dry-run", action="store_true", help="Report what would be fixed without modifying files"
    )

    p_bf = sub.add_parser(
        "backfill-brief", help="Populate brief_summary on entries that don't have one yet"
    )
    p_bf.add_argument(
    ...
    p_retrotag = sub.add_parser(
        "retrotag", help="Infer tags and areas for learnings that have neither"
    )
    p_retrotag.add_argument(
        "--dry-run", action="store_true", help="Print what would be tagged without writing"
    )
    p_retrotag.add_argument(
        "--model", default=None, help="Override the claude model used for inference"
    )
    p_retrotag.add_argument(
        "--limit",
        type=int,
    ...
    p_backup = sub.add_parser("backup", help="Snapshot every index.jsonl to backups/<YYYY-MM-DD>/")
    p_backup.add_argument(
        "--retain",
        type=int,
        default=maintenance.DEFAULT_RETAIN,
        help=f"Dated snapshots to keep (default {maintenance.DEFAULT_RETAIN}; "
        "0 keeps only the newest)",
    )

    p_restore = sub.add_parser("restore", help="Restore index.jsonl files from a dated snapshot")
    ...
    p_restore.add_argument(
        "--list", action="store_true", help="List available snapshot dates and exit"
    )
    p_restore.add_argument(
        "--dry-run", action="store_true", help="Report what would be restored without writing"
    )
