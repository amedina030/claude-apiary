...
def main():
    parser = argparse.ArgumentParser(description="Autonomous hardener — pipeline stage 5")
    ...
    branch = execution.get("branch", f"pipeline/{uuid}")
    ...
    try:
        for round_num in range(1, MAX_ROUNDS + 1):
            ...
            try:
                commit_all(f"pipeline/{uuid} harden round {round_num} fixes")
            except RuntimeError as e:
