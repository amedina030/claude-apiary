#!/usr/bin/env python3
"""Helpers for the detached cron-driven pipeline mode."""
...
def list_pipeline_branches() -> list:
    """Return list of local branch names starting with 'pipeline/'. Empty on error."""
    r = _git(['for-each-ref', '--format=%(refname:short)', 'refs/heads/pipeline/'])
    if r.returncode != 0:
        return []
    return [ln.strip() for ln in r.stdout.splitlines() if ln.strip()]

def list_unmerged_pipeline_branches(base: str = 'master') -> list:
    """Return pipeline/* branches not merged into base."""
    r = _git(['branch', '--no-merged', base, '--list', 'pipeline/*'])
    if r.returncode != 0:
        return []
    out = []
...
def hygiene_precheck(max_unreviewed: int, base: str = 'master') -> Optional[str]:
    ...
    branches = list_unmerged_pipeline_branches(base)
    if len(branches) >= max_unreviewed:
        return f'queue full ({len(branches)}/{max_unreviewed})'
    return None

def _branch_exists_for_uuid(uuid: str) -> bool:
    """True if any pipeline/* branch name contains the uuid."""
    for b in list_pipeline_branches():
        if uuid in b:
            return True
    return False

def pick_backlog_item() -> Optional[Path]:
    """Return oldest-mtime pipeline/backlog/*.json whose uuid has no existing pipeline/* branch. None if none."""
