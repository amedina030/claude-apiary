#!/usr/bin/env python3
"""Mark a backlog ticket as done without running it through the pipeline.

For tickets small enough to fix by hand. Deletes pipeline/backlog/<slug>.json
and updates the matching pipeline/board.md row's status column to 'done'.
Refuses to operate if the row is not currently in 'backlog' status, to avoid
clobbering pipeline-run state.
...
def main():
    ...
    if current_status != 'backlog':
        print(
            ...
            f"not 'backlog'. mark_done is only for tickets that never entered the pipeline.",
            file=sys.stderr,
        )
        sys.exit(1)
