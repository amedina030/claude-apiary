...
def build_prompt(spec: dict, previous_errors: list[str] | None = None) -> str:
    ...
    parts = [
        ...
        "PIPELINE-PACKAGE IMPORT CONVENTION: when generating test files under "
        "pipeline/ (e.g. pipeline/test_<module>.py) that will be run as "
        "'python -m unittest pipeline.test_<module>', do NOT use bare imports "
        "of sibling pipeline modules — `import detached_lib` will fail with "
        "ModuleNotFoundError because the file loads under the 'pipeline' "
        "package and sys.path has repo root, not pipeline/. Use either "
        "`from pipeline import detached_lib` OR the sys.path-insert pattern "
        "shown in pipeline/test_run_detached.py "
        "(_PIPELINE_DIR = Path(__file__).resolve().parent; "
        "sys.path.insert(0, str(_PIPELINE_DIR)); then bare `import detached_lib  # noqa: E402`). "
        "Do the same for any test action whose code_spec runs a pipeline test module.",
