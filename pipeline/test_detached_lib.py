...
class TestHygiene(unittest.TestCase):
    def test_ok_when_below_max(self):
        with mock.patch.object(detached_lib, 'list_unmerged_pipeline_branches', return_value=['pipeline/a-1', 'pipeline/b-2']):
            self.assertIsNone(detached_lib.hygiene_precheck(5))
    def test_full(self):
        with mock.patch.object(detached_lib, 'list_unmerged_pipeline_branches', return_value=['pipeline/a','pipeline/b','pipeline/c','pipeline/d','pipeline/e']):
    ...
    def test_picks_oldest_not_claimed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(detached_lib, 'BACKLOG_DIR', bdir):
                with mock.patch.object(detached_lib, 'list_pipeline_branches', return_value=[]):
                    p = detached_lib.pick_backlog_item()
                    self.assertEqual(p.name, 'a.json')
                with mock.patch.object(detached_lib, 'list_pipeline_branches', return_value=['pipeline/a-xxxxxxxx-uuid-a']):
                    p = detached_lib.pick_backlog_item()
