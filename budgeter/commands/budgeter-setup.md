...
2. Run setup via the launcher:
   ```bash
   python "$CLAUDE_PROJECT_DIR/.claude/apiary/launch.py" setup.py --project-path "<current-working-directory>"
   ```

   If the launcher fails (e.g. `"$CLAUDE_PROJECT_DIR/.claude/apiary/launch.py"` does not exist), the global install has not been run yet. Ask the user for the apiary repo path and run `python <path>/setup.py --global` first.
