...
(() => {
  ...
  // --- agents panel (T-2026-181) -------------------------------------------
  // Renders one row per active/recent subagent. Data comes from
  // window.apiary.onAgents() (backend polls <session>/subagents/*.jsonl).
  // Elapsed time updates locally via a 1s setInterval so we don't need a
  // backend round-trip just to tick the clock.
  const agentsPanelEl = document.getElementById("agents-panel");
  const agentsTitleEl = document.getElementById("agents-title");
  const agentsRunningEl = document.getElementById("agents-running");
  const agentsRecentEl = document.getElementById("agents-recent");
  const agentsRecentWrapEl = document.getElementById("agents-recent-wrap");
  const agentsBadgeEl = document.getElementById("agents-badge");
  const agentsClearBtnEl = document.getElementById("agents-clear-recent");
  let lastAgentsPayload = [];
  const agentsExpanded = new Set();
  // Dismissed per-agent — filtered out of the "recent" list. Persisted so a
  // reload doesn't resurrect agents the user already dismissed.
  ...
  let agentsCollapsed = (() => {
    try { return localStorage.getItem("apiary.agents.collapsed") === "1"; }
    catch (_) { return false; }
  })();
  function reflectAgentsCollapsed() {
    if (agentsPanelEl) agentsPanelEl.classList.toggle("collapsed", agentsCollapsed);
    if (agentsTitleEl) agentsTitleEl.setAttribute("aria-expanded", agentsCollapsed ? "false" : "true");
  }
  reflectAgentsCollapsed();
  if (agentsTitleEl) {
    const toggleAgentsCollapsed = () => {
      agentsCollapsed = !agentsCollapsed;
      try { localStorage.setItem("apiary.agents.collapsed", agentsCollapsed ? "1" : "0"); }
      catch (_) {}
      reflectAgentsCollapsed();
    };
    agentsTitleEl.addEventListener("click", toggleAgentsCollapsed);
    agentsTitleEl.addEventListener("keydown", (ev) => {
      if (ev.key === "Enter" || ev.key === " ") {
        ev.preventDefault();
  ...
  function fmtElapsed(startedAtEpoch, nowMs) {
  ...
  function fmtDuration(startedAt, lastActivity) {
  ...
  function renderAgentItem(agent) {
    const li = document.createElement("li");
    li.className = "agent-item" + (agentsExpanded.has(agent.agent_id) ? " expanded" : "");
    li.dataset.agentId = agent.agent_id;
    li.dataset.startedAt = String(agent.started_at || 0);
    li.dataset.status = agent.status;

    const row = document.createElement("div");
    // Only this row toggles expand — text below stays selectable.
    row.className = "agent-row agent-expand-target";
    const dot = document.createElement("span");
    dot.className = "agent-dot " + (agent.status || "running");
    dot.textContent = agent.status === "done" ? "○" : agent.status === "failed" ? "✕" : "●";
    row.appendChild(dot);
    ...
    if (agent.model) {
      ...
      mb.className = "agent-model-badge";
      // Compact: "claude-sonnet-4-6" → "sonnet"
      const m = /claude-(opus|sonnet|haiku)/i.exec(agent.model);
      mb.textContent = m ? m[1].toLowerCase() : agent.model.slice(0, 8);
      row.appendChild(mb);
    }

    const meta = document.createElement("span");
    meta.className = "agent-meta";
    const tot = (agent.tokens.input | 0) + (agent.tokens.output | 0) + (agent.tokens.cache_read | 0) + (agent.tokens.cache_write | 0);
    if (agent.status === "running") {
      meta.textContent = fmtElapsed(agent.started_at, Date.now()) + " · " + fmtTokens(tot);
    } else {
      meta.textContent = fmtDuration(agent.started_at, agent.last_activity_at) + " · " + fmtTokens(tot);
    }
    row.appendChild(meta);

    // Per-row dismiss × for completed/failed agents.
    if (agent.status !== "running") {
      const dismiss = document.createElement("button");
      dismiss.type = "button";
      dismiss.className = "agent-dismiss-btn";
      dismiss.setAttribute("aria-label", "dismiss this agent");
      dismiss.title = "dismiss";
      dismiss.textContent = "×";
      dismiss.addEventListener("click", (ev) => {
        ev.stopPropagation();
        agentsDismissed.add(agent.agent_id);
        persistDismissed();
        renderAgents(lastAgentsPayload);
      });
    ...
    if (agent.status === "running" && agent.current_tool) {
      ...
      tl.className = "agent-tool-line";
      const curT = document.createElement("span");
      curT.className = "cur-tool";
      curT.textContent = agent.current_tool;
    ...
    if (histEntries.length) {
      ...
      row1.className = "kv";
      const k = document.createElement("span"); k.className = "k"; k.textContent = "tools";
      const v = document.createElement("span"); v.className = "v";
      v.textContent = histEntries.map(([name, n]) => n + " " + name).join(" · ");
      row1.appendChild(k); row1.appendChild(v);
      detail.appendChild(row1);
    }
    const mkKv = (label, val) => {
    ...
    detail.appendChild(mkKv("in", fmtTokens(agent.tokens.input)));
    detail.appendChild(mkKv("out", fmtTokens(agent.tokens.output)));
    detail.appendChild(mkKv("cache r", fmtTokens(agent.tokens.cache_read)));
    detail.appendChild(mkKv("cache w", fmtTokens(agent.tokens.cache_write)));
    if (agent.prompt_preview) {
      detail.appendChild(mkKv("prompt", agent.prompt_preview));
    }
    if (agent.final_text && agent.status === "done") {
      const fin = document.createElement("div");
      fin.className = "agent-final";
  ...
  function renderAgents(payload) {
    ...
    if (agentsRunningEl) {
      ...
      if (running.length === 0 && recent.length === 0) {
        ...
        agentsRunningEl.appendChild(empty);
      } else {
        for (const a of running) agentsRunningEl.appendChild(renderAgentItem(a));
      }
    }
    if (agentsRecentEl) {
      agentsRecentEl.innerHTML = "";
      for (const a of recent) agentsRecentEl.appendChild(renderAgentItem(a));
    }
    if (agentsRecentWrapEl) agentsRecentWrapEl.classList.toggle("hidden", recent.length === 0);
    if (agentsClearBtnEl) agentsClearBtnEl.classList.toggle("hidden", recent.length === 0);
    if (agentsBadgeEl) {
      if (running.length === 0 && recent.length === 0) {
        agentsBadgeEl.textContent = "";
      } else if (running.length) {
        agentsBadgeEl.textContent = running.length + " running" + (recent.length ? ", " + recent.length + " done" : "");
      } else {
        agentsBadgeEl.textContent = recent.length + " done";
      }
    }
  }
  // Delegate expand/collapse clicks — only the top row toggles, so text in
  // the expanded detail stays selectable for copy-paste.
  if (agentsPanelEl) {
    agentsPanelEl.addEventListener("click", (ev) => {
      if (!ev.target.closest(".agent-expand-target")) return;
      const item = ev.target.closest(".agent-item");
      if (!item) return;
      const id = item.dataset.agentId;
      if (!id) return;
      if (agentsExpanded.has(id)) agentsExpanded.delete(id);
      else agentsExpanded.add(id);
  ...
  // Live clock: re-render every second so elapsed strings tick for running
  // agents. Cheap — typical agent count is <5.
  setInterval(() => {
    if (!agentsPanelEl) return;
    const hasRunning = lastAgentsPayload.some((a) => a && a.status === "running");
    if (hasRunning) renderAgents(lastAgentsPayload);
  }, 1000);
  // Seed with empty state so the panel shows "no agents running" on first load.
  renderAgents([]);
  ...
  window.apiary = {
    setActiveSession(sid) {
      ...
      try { recentClearedAt = 0; agentsExpanded.clear(); renderAgents([]); } catch (_) {}
    },
    ...
    onAgents(arrJson, sessionId) {
      ...
      try {
        const arr = typeof arrJson === "string" ? JSON.parse(arrJson) : arrJson;
        renderAgents(Array.isArray(arr) ? arr : []);
      } catch (e) {
