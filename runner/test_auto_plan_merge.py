...
def _step(
    num, action, files, depends_on=None, code_spec="spec", description="d", post_conditions=None
):
    s = {
        "step_number": num,
        "type": action,
...
class MergeSubsumedStepsTests(unittest.TestCase):
    ...
    def test_does_not_merge_test_or_verify(self):
        steps = [
            _step(1, "create", ["a.py"]),
            _step(
                2, "test", ["a.py"], depends_on=[1], code_spec="python -m unittest runner.test_a"
            ),
        ]
        out = _merge_subsumed_steps(steps)
        self.assertEqual(len(out), 2)
    ...
    def test_renumbers_and_remaps_downstream_deps(self):
        steps = [
            ...
            _step(
                4, "test", ["b.py"], depends_on=[3], code_spec="python -m unittest runner.test_b"
            ),
        ]
        out = _merge_subsumed_steps(steps)
