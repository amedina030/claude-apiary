...
If the current target's compass rule table exists, read it as part of loading this context. It is written in the second person, to you: how this user wants decisions made, output written and questions pre-empted, mined from their own corrections and acceptances (D-2026-62). Apply its self-check before finalizing a recommendation or report.

```bash
state_dir=$(python "$(git rev-parse --show-toplevel)/.claude/apiary/launch.py" core/utils/state.py)
test -f "$state_dir/compass/rules.md" && cat "$state_dir/compass/rules.md"
```

If the file is missing, do nothing (no error, no fallback). The table is regenerated by `compass/rules.py build` (which `/wrapup` runs after classifying the session's turn pairs); never edit it by hand — rows go in `<state-dir>/compass/rules_manual.json`. Explicit user statements and `feedback`-type memory entries override any row.
