...
| PreToolUse (`pre`) | 3 | `learnings_inject` | `core/hooks/learnings_inject_hook.py` | `Edit\|Write\|Bash` | Injects the top-3 most-relevant learnings before an Edit/Write/Bash, scored against the tool payload (file paths via area globs, command tokens via tags; catch-all globs like `**/*.py` rank below subsystem globs, ties go to the newest learning). Each learning is injected at most once per session (seen-file under `session-tmp/`). **On by default**; per-repo kill switch: `core/flags.py enable learnings-inject-off`. |
