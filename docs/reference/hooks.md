...
| UserPromptSubmit (`prompt`) | 2 | `compass_rules` | `core/hooks/compass_rules.py` | _(every tool)_ | The per-turn pin: on every user message after the first, injects the compact form of `<state-dir>/compass/rules.md` — the principle rows plus the self-check, about 200 tokens — so the rules sit next to the turn being composed and survive compaction. The first message is skipped (its own flag under `session-tmp/`) because the startup block carries the whole table. Nothing when the target has no `rules.md`. |
