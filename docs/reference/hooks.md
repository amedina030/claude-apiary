...
| PreToolUse (`pre`) | 8 | `budgeter_pre` | `budgeter/hooks/pre_tool_use.py` | `Agent\|Bash\|Read\|Write` | Logs the previous tool call's token cost (PRE-to-PRE delta) and injects the session-length nudge once per tier. The matcher is the `monitored_tools` alternation read from `budgeter/config.json` at dispatch time. |
