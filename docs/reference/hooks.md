...
| PreToolUse (`pre`) | 3 | `learnings_inject` | `core/hooks/learnings_inject_hook.py` | `Edit\|Write\|Bash` | Injects the top-3 most-relevant learnings before an Edit/Write/Bash, scored against the tool payload (file paths via area globs; Bash command tokens via tags, matched **whole-token** with the launcher idiom stripped first, so `api` does not fire on "apiary" nor `git` on every launcher call; catch-all globs like `**/*.py` rank below subsystem globs, ties go to the newest learning). Each learning is injected at most once per session — per session *and* agent when the payload carries an `agent_id` (seen-file under `session-tmp/`). Not skipped for runner subprocesses: the runner skip is for the startup dump, and a headless executor is where a known-trap note pays off. **On by default**; per-repo kill switch: `core/flags.py enable learnings-inject-off`. |
...
| PreToolUse (`pre`) | 5 | `compass_rules` | `core/hooks/compass_rules.py` | `Agent\|Task\|AskUserQuestion` | The hook-point half of the compass rule table (D-2026-62, the minor delivery path): before an `Agent`/`Task` spawn injects rule J5 (usage cost) once per session and agent (flag under `session-tmp/`), before `AskUserQuestion` injects O3 (ask in prose) every time. The rule text is read from `<state-dir>/compass/rules.md` by id, so a manual override changes what is injected and a dropped id injects nothing. Skipped for runner subprocesses, which get the table as a prompt preamble from `runner/claude_subprocess.py`. |
| PreToolUse (`pre`) | 6 | `pre_push_doc_conformer` | `core/hooks/pre_push_doc_conformer.py` | `Bash` | On a Bash `git push`, runs the pushed repo's `docs/check_cli_claims.py` and **blocks the push** with the drift report as the reason if it exits nonzero. Inert in repos that do not ship the conformer. Fails open on internal errors — only a clean nonzero exit blocks. |
| PreToolUse (`pre`) | 7 | `pre_push_secret_scan` | `core/hooks/pre_push_secret_scan.py` | `Bash` | On a Bash `git push`, works out what is being pushed (the named ref, every branch for `--all`/`--mirror`, else `HEAD`; honours `git -C <dir>`), then scans the added lines of **every outgoing commit individually**, so a secret committed and later deleted is still caught. Rules come from `core/secret_patterns.py`, shared with the commit-time gate. **Blocks** on a hit, reporting `file:line @commit` with the value redacted. Whitelist with `apiary:allow-secret` on the line or an entry in `.secretsallow`. A scan that starts but cannot finish blocks and says so. |
| PreToolUse (`pre`) | 8 | `budgeter_pre` | `budgeter/hooks/pre_tool_use.py` | `Agent\|Bash\|Read\|Write` | Logs the previous tool call's token cost (PRE-to-PRE delta), injects the session-length nudge once per tier, and injects the usage-limit nudge when the newest sample in `budgeter/data/usage_samples.jsonl` shows the 5-hour or 7-day limit past its configured threshold (on by default, silenced per repo by `budgeter-usage-warn-off`). The matcher is the `monitored_tools` alternation read from `budgeter/config.json` at dispatch time. |
| PreToolUse (`pre`) | 9 | `remind_standards` | `docs/hooks/remind_standards.py` | `Write\|Edit` | On the session's first Write/Edit of a `.py` file or a `docs/*.md` file, injects a one-line pointer to the relevant standards doc. Once per file category per session. Classification is relative to **the repo the write is in** (`CLAUDE_PROJECT_DIR`, then `APIARY_TARGET_REPO`, then the payload cwd), so it works in every bootstrapped repo; "new tool" means the top-level directory holds no other Python (T-2026-282). |
| PreToolUse (`pre`) | 10 | `prose_check` | `prose/hooks/pre_tool_use.py` | `Write\|Edit\|Bash` | The prose linter at the moment text is produced (`docs/standards/prose-style.md`). On the session's first markdown `Write`/`Edit` or first `scribe/notes.py add`/`learn` Bash command, injects the rule list once (rendered from the rule files, flag under `session-tmp/`). On every included markdown `Write` (`content`) or `Edit` (`new_string`), runs `prose/engine.py` and injects the warning-or-above findings as context, capped at 12. **Blocks** only when the per-repo `prose-gate` flag is on and an error-level finding exists. Paths come from `prose/config.json` `include_globs`/`exclude_globs` (plus `<repo>/.claude/prose.json`). Scribe note bodies are checked by `notes.py` itself. Skipped for runner subprocesses; fails open. |
...
| Stop (`stop`) | 3 | `compass_pair_log` | `core/hooks/compass_pair_log.py` | _(every tool)_ | Appends the `(assistant_text, user_turn, ts)` pairs the transcript gained this turn to `<state-dir>/compass/turns/<sid>.jsonl` for the compass rule table (D-2026-62). A byte-offset cursor beside the file keeps each call proportional to the turn; the record filter keeps only real user prompts (`promptId`, string content, no attachment, not a sidechain, not `entrypoint == "sdk-cli"`, not a task notification or slash-command invocation). The same walk scores the turn's final assistant message with the output heuristics (`compass/heuristics.py`: outcome first, one recommendation, length band) into `events/<sid>.heuristics.jsonl`. No model call, never adds context. Skipped for runner subprocesses. `compass/classify.py` reads the pairs at `/wrapup` or in the nightly catch-up. |
| UserPromptSubmit (`prompt`) | 1 | `startup_prompt` | `core/hooks/startup_prompt_hook.py` | _(every tool)_ | On the first user message, injects identity, the notes summary, the learnings index, the CLI index, the apiary toolkit rules and the compass rule table (`<state-dir>/compass/rules.md`, D-2026-62). With `APIARY_GUI_SESSION=1` it also injects a `surface:` note saying the session runs inside the GUI. |
| UserPromptSubmit (`prompt`) | 2 | `compass_rules` | `core/hooks/compass_rules.py` | _(every tool)_ | The pin: on every tenth user message (`PIN_EVERY`), injects the compact form of `<state-dir>/compass/rules.md` — the principle rows plus the self-check, about 250 tokens — so the rules stay within reach of the turn being composed and survive compaction. The hook keeps the session's message count in a flag file under `session-tmp/` and does the counting itself; the first message never pins because the startup block carries the whole table. Nothing when the target has no `rules.md`. |
| UserPromptSubmit (`prompt`) | 3 | `telephone_grant` | `telephone/hooks/user_prompt.py` | _(every tool)_ | Writes the per-session grant that unlocks a cross-repo call's act mode, whenever the submitted prompt opens with `/telephone`. The grant records whether the user typed `act`, and `telephone/cli.py` consumes it (reads and deletes) on the first `call` or `reply`, so one typed command buys one privileged call. This is the whole authorization model, because the model can run the CLI from Bash but cannot make the user's keystrokes fire a hook. Failing open therefore fails closed: a hook that raised, or was never installed, leaves no grant and act mode stays refused. |
