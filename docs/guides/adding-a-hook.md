...
The settings.json entry itself is generated by `core/hooks_factory.py` through
`core/hooks_lib.hook_cmd` — never hand-write it. `hook_cmd` appends the
`# claude-apiary` ownership mark that `is_apiary_entry` uses to tell apiary's
entries from the user's; an unmarked entry is never refreshed on re-install and
never removed on uninstall.
